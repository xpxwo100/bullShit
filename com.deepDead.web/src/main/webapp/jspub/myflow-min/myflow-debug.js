...
(function(b) {
	...
	a.rect = function(p, m) {
		...
		b([ t.node, e.node ]).bind("mouseover",function(event){
			u.repositionNodeButtons();
		});
		var j = function(o, r) {
			if (!a.config.editable) {
		...
		var c = function(o, F, r) {
			if (r.getId() == g) {
				...
				var prePaths = r.getPreviousPaths();
				for(var cur = 0; cur < prePaths.length ; cur++ ){
					var curpath = prePaths[cur];
					var curpathText = curpath.text();
					if(curpathText.indexOf("TO") == 0){
						var newText = "TO "+ F;
						curpath.setText(newText);
						var fromRect = curpath.from();
						var expr = fromRect.getDecisionExpr();
						if(expr){
							var falseValue = expr.falseValue, trueValue = expr.trueValue, displayValue = expr.value;
							if(falseValue && falseValue == curpathText){
								expr.falseValue = newText;
								expr.value = displayValue.replace(curpathText,newText);
								fromRect.setDecisionExpr(expr);
							}else if(trueValue && trueValue == curpathText){
								expr.trueValue = newText;
								expr.value = displayValue.replace(curpathText,newText);
								fromRect.setDecisionExpr(expr);
							}
						}
		...
		this.toJson = function() {
			...
			for ( var o in E.props) {
				if(o == 'assignee'){
					r += o + ":{value:'" + E.props[o].value + "',dealValue:'" + E.props[o].dealValue + "',submitValue:'" + E.props[o].submitValue + "'},";
				}else if(o == 'condition'){
					var conditionSource = E.props[o].conditionSource;
					if(conditionSource instanceof Array){
						var conditionSourceStr = "[";
						for(var i = 0;i < conditionSource.length; i++){
							conditionSourceStr += ("{filedLabel : '" + conditionSource[i].filedLabel + "' ,filedValue : '"+ conditionSource[i].filedValue +"'}");
							if((i + 1) !=  conditionSource.length){
								conditionSourceStr += ",";
							}
						}
						conditionSourceStr += "]";
						conditionSource = conditionSourceStr;
					}
					if(conditionSource == ""){
						conditionSource = "''";
					}
					r += o + ":{value:'" + E.props[o].value + "',conditionSource:" + conditionSource + ",submitValue:'" + E.props[o].submitValue + "'},";
				}else if(o == 'expr'){
					r += o + ":{value:'" + E.props[o].value + "',trueValue:'" + E.props[o].trueValue + "',falseValue:'" + E.props[o].falseValue + "'},";
				}else if(o == 'moreUserSelect'){
					r += o + ":{value:'" + E.props[o].value + "',dealValue:'" + E.props[o].dealValue + "',submitValue:'" + E.props[o].submitValue + "'},";
				}else{
			...
			r += "},";
			//获取上级节点信息
			var previousPaths = this.getPreviousPaths();
			if(previousPaths.length == 1){
				var previousRect = previousPaths[0].from();
				if(previousRect){
					r += "formType:'"+ previousRect.getType() +"',";
				}
			}
			//获取下级节点信息
			var nextPaths = this.getNextPaths();
			if(nextPaths.length == 1){
				var nextRect = nextPaths[0].to();
				if(nextRect){
					var nextRectType = nextRect.getType();
					r += "toType:'"+ nextRectType +"',";
					//如果下级节点为人工决策，则继续获取下级信息，组装即将流向下拉框数据源
					if(nextRectType == 'acTask'){
						var next2Paths = nextRect.getNextPaths();
						if(next2Paths.length > 0 ){
							var acTaskData = "[";
							for(np=0; np < next2Paths.length; np++){
								var next2Path = next2Paths[np];
								var next2Rect = next2Paths[np].to();
								if(next2Path && next2Rect){
									var assignee = next2Rect.getAssigneeInfo();
									var userInfo = ""
									if(assignee && assignee.dealValue){
										userInfo = (assignee.dealValue=='user'?' ('+assignee.value+')':'');
									}
									acTaskData += ("{filedLabel : '" + next2Path.text() + ' ' + next2Rect.text()+ userInfo + "' ,filedValue : '"+ next2Path.text() +"'}");
									if((i+1) != next2Paths.length){
										acTaskData += ",";
									}
								}
							}
							acTaskData += "]";
							r += "acTaskData:"+ acTaskData;
						}
					}
				}
			}
		...
		this.getAssigneeInfo = function() {
			return E.props['assignee'];
		};
		...
		this.getPreviousPaths = function() {
			var self = this , rpaths = [];
			for(var pathName in a.allPaths){
				var rpath = a.allPaths[pathName];
				if(rpath && self.getId() == rpath.to().getId()){
					rpaths.push(rpath);
				}
			}
			return rpaths;
		};
		this.getPreviousPathObjects = function() {
			var self = this , rpathObjects = [];
			for(var pathName in a.allPaths){
				var rpath = a.allPaths[pathName];
				if(rpath && self.getId() == rpath.to().getId()){
					rpathObjects.push(eval("("+rpath.toJson()+")"));
				}
			}
			return rpathObjects;
		};
		this.getNextPaths = function() {
			...
			for(var pathName in a.allPaths){
				var rpath = a.allPaths[pathName];
				if(rpath && self.getId() == rpath.from().getId()){
					rpaths.push(rpath);
				}
			}
			return rpaths;
		};
		this.getNextPathObjects = function() {
			var self = this , rpathObjects = [];
			for(var pathName in a.allPaths){
				var rpath = a.allPaths[pathName];
				if(rpath && self.getId() == rpath.from().getId()){
		...
		this.makeNodeButtons = function(){
			var self = this,
			buttons = b('<div class="node-buttons" id="div'+u.getId()+'"><a href="javascript:void(0);" title="复制" class="icon icon-copy"></a> <a href="javascript:void(0);" title="删除" class="icon icon-delete"></a></div>').appendTo("body");
			return b("a.icon-copy", buttons).bind("click", function(){
				}),
				b("a.icon-delete", buttons).bind("click", function(){
					b(C).trigger("removerect", u);
				}),
				buttons;
		};
		this.repositionNodeButtons = function(){
			b(".node-buttons").hide();
			var btn = u._buttons || (u._buttons = u.makeNodeButtons()),
			targetBox = u.getBBox(),c=true;
			btn.css({
				left: targetBox.x + targetBox.width,
				top: targetBox.y,
				width: c ? 19 : 38
			}).attr("for", u.getId()).show().find("a:first")[c ? "hide" : "show"]();
		};
		this.hideNodeButtons = function(){
			this._buttons && this._buttons.hide();
		};
		this.getDecisionExpr = function() {
			return E.props['expr'];
		};
		this.setDecisionExpr = function(expr) {
			E.props['expr'] = expr;
		};
		B();
	};
	a.path = function(q, n, u, e) {
		...
		b([ i.node, t.node ]).bind("mouseover",function(event){
			v.repositionNodeButtons();
		});
		var l = function(r, C) {
			if (!a.config.editable) {
		...
		this.makeNodeButtons = function(){
			var self = this,
			buttons = b('<div class="node-buttons" id="div'+self.getId()+'"><a href="javascript:void(0);" title="复制" class="icon icon-copy"></a> <a href="javascript:void(0);" title="删除" class="icon icon-delete"></a></div>').appendTo("body");
			return b("a.icon-copy", buttons).bind("click", function(){
				}),
				b("a.icon-delete", buttons).bind("click", function(){
					//self.remove();
		...
		this.repositionNodeButtons = function(){
			b(".node-buttons").hide();
			var btn = this._buttons || (this._buttons = this.makeNodeButtons()),
			targetBox = x.midDot().pos(),c=true;
			btn.css({
				left: targetBox.x+25,
				top: targetBox.y-10,
				width: c ? 19 : 38
			}).attr("for", this.getId()).show().find("a:first")[c ? "hide" : "show"]();
		};
		this.hideNodeButtons = function(){
			this._buttons && this._buttons.hide();
		};
	...
	a.props = function(h, f) {
		...
		var d = function(n, m, o) {
			...
			c.show();
			for ( var l in m) {
				e.append("<tr><th width='30%'>" + m[l].label + '</th><td><div id="p' + l
	...
	a.editors = {
		textEditor : function() {
			...
			this.init = function(i, h, m, l, j) {
				...
				if(d[e].readonly){
					strInput = '<input  style="width:100%;background:#DDD;" readonly ="'+d[e].readonly+'"/>';
				}
				b(strInput).val(g.text()).change(
						function() {
...
//		});
	     b(window).scroll(function(){
		        ...
		        var toolposition = a.config.tools.attr;
		        var propsposition = a.config.props.attr;
		        timer = setInterval(function(){
		             b("#myflow_tools").animate({"top": scroll.top + toolposition.top,"left": toolposition.left + scroll.left},0);
		             b("#myflow_props").animate({"top": scroll.top + propsposition.top,"left": window.outerWidth - b("#myflow_props").width() - 20 + scroll.left},0);
				     clearInterval(timer);
		        },50);
		   });
		var w = function(c, i) {
		...
		if (a.config.editable) {
			...
			b(x).droppable({
				...
				drop : function(c, i) {
					b(y).trigger("addrect", [ i.helper.attr("type"), {
						attr : {
							x : i.helper.offset().left-10,
							y : i.helper.offset().top-30
						}
					} ])
				}
			});
			b("#myflow_save").click(
					function() {
						b("table").find(".editor").each(function() {
							var k = b(this).data("editor");
							if (k) {
								k.destroy()
							}
    ...
    function getScroll(){
        ...
        return {"top":bodyTop,"left":bodyLeft};
   }

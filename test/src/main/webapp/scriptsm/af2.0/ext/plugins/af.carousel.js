/**
 * 该侧滑插件来自jqmobi1.*,2.0中未找到该插件
 *2013/6/4做相应修改 by tangmingbao1@ishitong.com
 *解决可设置是否可循环侧滑
 *解决和scroller的冲突
 * 
 *
		...
		carousel.prototype = {
				...
				touchMove: function(e) {
					...
					if (this.vertical) {
						...
						{
							//非循环侧滑处理
							if(this.carouselIndex==0&&rawDelta.x>0)
							{
								//不允许第一个元素向右滑
								return this.lockMove=true;
							}
							if(this.carouselIndex==(this.childrenCount-1)&&rawDelta.x<0)
							{
								//不允许最后一个元素向左滑
								return this.lockMove=true;
							}
						...
						if ((!this.lockMove&&isHorizontalSwipe(rawDelta.x, rawDelta.y))||Math.abs(this.dx)>5) {
							...
							//console.info("carousel:pageX"+e.touches[0].pageX+" startX:"+this.startX+" cssMoveStart:"+this.cssMoveStart);
							e.preventDefault();
							//  e.stopPropagation();
							movePos.x = this.dx;
						}
					...
					{
						//console.info("craouse move:"+movePos.x)
